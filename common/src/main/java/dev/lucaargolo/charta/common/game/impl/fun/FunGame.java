...
public class FunGame extends Game<FunGame, FunMenu> {
    ...
    public Predicate<Deck> getDeckPredicate() {
        return (deck) -> {
            return deck.getCards().size() >= 108 && Suits.FUN.containsAll(deck.getSuits()) && deck.getSuits().containsAll(Suits.FUN);
        };
    }

    @Override
    public Predicate<Card> getCardPredicate() {
        return (card) -> Suits.FUN.contains(card.suit()) && Ranks.FUN.contains(card.rank());
    }
            ...
            if(play == null) {
                ...
                if(getFullHand(currentPlayer).findAny().isEmpty()) {
                    ...
                    if(currentPlayer.shouldCompute()) {
                        ...
                        isChoosingWild = true;
                        suits.clear();
                        suits.addAll(gameSuits.stream().map(suit -> Card.create(suit, Ranks.ZERO)).toList());
                    ...
                    }else if(card.rank() == Ranks.REVERSE) {
                    ...
                    }else if(card.rank() == Ranks.PLUS_2) {
    ...
    public boolean canPlay(CardPlayer player, GamePlay play) {
        ...
        if(drawStack > 0) {
            ...
            if(startedDraw) {
                ...
                boolean isPlus4 = lastCard.rank() == Ranks.WILD_PLUS_4;
                if (isPlus4) {
                    return (STACK_PLUS4_ON_PLUS4.get() && card.rank() == Ranks.WILD_PLUS_4) || (STACK_ANY_PLUS2_ON_PLUS4.get() && card.rank() == Ranks.PLUS_2) || (STACK_SAME_PLUS2_ON_PLUS4.get() && card.rank() == Ranks.PLUS_2 && card.suit() == currentSuit);
                } else {
                    return (STACK_PLUS4_ON_PLUS2.get() && card.rank() == Ranks.WILD_PLUS_4) || (STACK_ANY_PLUS2_ON_PLUS2.get() && card.rank() == Ranks.PLUS_2) || (STACK_SAME_PLUS2_ON_PLUS2.get() && card.rank() == Ranks.PLUS_2 && card.suit() == currentSuit);
                }
            }
        }

        //If there aren't any edge cases, we check if the card is a wild card, or if it matches the current rank or suit.
        return card.rank() == Ranks.WILD || card.rank() == Ranks.WILD_PLUS_4 || card.rank() == lastCard.rank() || card.suit() == currentSuit;
    }
