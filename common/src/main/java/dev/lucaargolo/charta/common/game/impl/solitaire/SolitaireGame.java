...
public class SolitaireGame extends Game<SolitaireGame, SolitaireMenu> {
    ...
    public SolitaireGame(List<CardPlayer> players, Deck deck) {
        ...
        for(Suit suit : List.of(Suits.SPADES, Suits.HEARTS, Suits.CLUBS, Suits.DIAMONDS)) {
            ...
            GameSlot slot = addSlot(new GameSlot(new LinkedList<>(), leftX + CardImage.WIDTH + (CardImage.WIDTH + 5)*i, topY + 5f + CardImage.HEIGHT - (CardImage.HEIGHT*1.5f), 0f, 180f, Direction.NORTH, CardImage.HEIGHT*3, false)  {
                ...
                public boolean canInsertCard(CardPlayer player, List<Card> cards, int index) {
                    ...
                    for(Card current : cards) {
                        if((last == null && current.rank() != Ranks.KING) || ((last != null && !SolitaireGame.isAlternate(last, current)) || (last != null && current.rank().ordinal()+1 != last.rank().ordinal()))) {
                            return false;
                        }
                        last = current;
                    }
    ...
    public static boolean isAlternate(Card c1, Card c2) {
        boolean v1 = (c1.suit() == Suits.HEARTS || c1.suit() == Suits.DIAMONDS);
        boolean v2 = (c2.suit() == Suits.HEARTS || c2.suit() == Suits.DIAMONDS);
        return v1 != v2;
    }
